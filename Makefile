...
#   make sim            -> just simulate rtl/, without rebuilding it first
...
# --- RTL simulation run only, against whatever's already compiled in
# rtl/build/work -- does NOT rebuild first (rtl's "run" target only
