...
#
# Every top-level target here is independent -- none of them pull
# each other in implicitly. Pick the one that does what you want:
#
#   make                 -> submodules + all packages (NO rtl build)
#   make packages        -> just the C++/Python tool packages
#   make rtl              -> just the RTL build (vlib/vcom/vlog via rtl/Makefile)
#   make sim               -> simulate rtl/'s already-compiled work lib (GUI)
#   make rtl-project       -> RTL build via ModelSim's .mpf project flow instead
#   make rtl-project-run   -> simulate via that .mpf project
...
#   make run ...          -> full network-to-bitstream-input pipeline (builds
#                            packages AND rtl, then simulates), see below
...
.PHONY: all submodules packages $(PACKAGE_DIRS) rtl sim \
...
# (or "make rtl-project") separately when you want the RTL build too,
# or "make run" for the full pipeline (which does include rtl).
all: submodules packages
...
# --- RTL ModelSim *project* (.mpf) flow -- a completely separate,
# alternative path to "rtl"/"sim" above: it is never invoked by "all",
# "run", or any other target here, and it doesn't invoke them either.
# Forwards to rtl/Makefile's own project/project-run targets, which
