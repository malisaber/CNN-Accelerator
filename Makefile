...
#   make rtl-simulate		-> simulate via that .mpf project
...
#   make firmware        	-> full network-to-bitstream-input pipeline (builds packages AND rtl, then simulates), see below
...
.PHONY: all submodules packages $(PACKAGE_DIRS) \
        rtl-project rtl-simulate \
...
# Simulation
rtl-simulate:
	$(call LOG,rtl-simulate,$(MAKE) -C rtl simulate)
