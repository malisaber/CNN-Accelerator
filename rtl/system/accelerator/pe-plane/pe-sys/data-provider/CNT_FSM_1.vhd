...
architecture Behavioral of CNT_FSM_1 is
	--------------------------------------------------------------------------
	--------------------------------------------------------------------------
	--		TYPE
	--------------------------------------------------------------------------
	TYPE	FSM							IS	(L0, L1, L2, L3, L4, L5, L6, L7, L8);
	--------------------------------------------------------------------------
	--------------------------------------------------------------------------
	--		SIGNALS
	--------------------------------------------------------------------------
...
begin
	...
	BEGIN
		...
		CASE	P_S	IS
			...
			WHEN	L3					=>													N_S	<=	L4;
			WHEN	L4					=>	IF	Kern_eq = '1'	AND	Colm_eq = '1'	THEN	N_S	<=	L5;						END IF;
			WHEN	L5					=>	IF	Chan_eq = '1'						THEN	N_S	<=	L6;	ELSE	N_S	<=	L3;	END	IF;
			WHEN	L6					=>	IF	pipo_ready = '1'					THEN	N_S	<=	L7;						END IF;
			WHEN	L7					=>	IF	Rows_eq = '1'						THEN	N_S	<=	L8;	ELSE	N_S	<=	L2;	END	IF;
			WHEN	L8					=>													N_S	<=	L1;
		END CASE;
	END PROCESS;
	--------------------------------------------------------------------------
